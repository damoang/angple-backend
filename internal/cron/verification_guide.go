...
func runVerificationGuide(db *gorm.DB) (*VerificationGuideResult, error) {
	...
	for _, t := range targets {
		...
		// 문구 주의: 난수만 알려주면 신청자가 "다음에 뭘 해야 하는지" 몰라 멈춘다
		// (2026-07-25 실측 — 난수만 메일로 회신하고 대기하신 사례 2건).
		// 공지 #45 의 전체 절차(장소 지정 → 사진 촬영 → 업로드)를 댓글에 담는다.
		content := fmt.Sprintf(
			`<p>안녕하세요, 다모앙입니다. 🙇 해외 실명인증 신청 감사합니다.</p>`+
				`<p><b>[인증 난수] %d</b></p>`+
				`<p>아래 순서로 진행해 주세요.</p>`+
				`<ol>`+
				`<li>신청하신 <b>장소 3곳 중 1곳</b>을 방문해 주세요.</li>`+
				`<li>종이에 <b>본인 다모앙 아이디와 위 인증 난수</b>를 적어주세요.</li>`+
				`<li><b>장소의 간판과 종이가 함께 보이도록</b> 사진 1장을 촬영해 주세요. `+
				`(휴대폰 사진 <b>위치정보를 켠 상태</b>로 촬영)</li>`+
				`<li>촬영한 <b>사진 원본</b>과 <b>사진 상세정보(위치) 스크린샷</b>을 `+
				`이 글에 댓글로 첨부하거나 security@damoang.net 으로 보내주세요.</li>`+
				`</ol>`+
				`<p>아직 장소를 지정하지 않으셨다면, 구글맵에 등록된 동네 10km 이내 장소`+
				`(편의점·주유소·은행 등 간판이 잘 보이는 곳) 3곳을 먼저 알려주세요.</p>`+
				`<p>문의: security@damoang.net</p>`,
			nansu,
		)
