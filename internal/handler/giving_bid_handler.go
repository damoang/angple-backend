...
// loadGivingMeta returns the meta row or a synthesized default (lowest_unique/open).
func (h *GivingHandler) loadGivingMeta(wrID int) givingMetaRow {
	var meta givingMetaRow
	err := h.db.Table("g5_giving_meta").Where("wr_id = ?", wrID).Take(&meta).Error
	if err != nil {
		return givingMetaRow{WrID: wrID, Method: givingdomain.DefaultMethod, Status: "open"}
	}
...
func (h *GivingHandler) AdminAction(c *gin.Context) {
	...
	case "force-stop":
		...
		meta := h.loadGivingMeta(wrID)
		if givingdomain.IsAutoDraw(meta.Method) {
			var existing int64
